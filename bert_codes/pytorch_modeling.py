...
class BertForTokenClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))  # (batch*length,dim) & (batch*length)
            return loss
        else:
            return logits
...
class BertForQA_CLS(PreTrainedBertModel):
    def __init__(self, config, num_labels):
        ...
        self.bert = BertModel(config)
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
        self.cls_outputs = nn.Linear(config.hidden_size, num_labels)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, target_labels=None, cls_weight=None):
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        logits = self.qa_outputs(sequence_output)
        start_logits, end_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1)  # [bs, len]
        end_logits = end_logits.squeeze(-1)  # [bs, len]
        target_logits = self.cls_outputs(pooled_output).squeeze(dim=-1)  # [bs,]
        ...
        if start_positions is not None and end_positions is not None:
            ...
            if cls_weight is not None:
                loss_fct_cls = BCEWithLogitsLoss(pos_weight=cls_weight)
            else:
                loss_fct_cls = BCEWithLogitsLoss()
            cls_loss = loss_fct_cls(target_logits, target_labels)
        
            total_loss = cls_loss + (start_loss + end_loss) / 2
            return total_loss
        else:
